...
async function changeVote({ idUsuario, idCeremonia, nominacionId }) {
  const redis = getRedisClient();
  const isClosed = await redis.exists(`ceremony:closed:${idCeremonia}`);
  if (isClosed) {
    throw new HttpError(409, "Las votaciones de esta ceremonia ya estan cerradas.");
  ...
  try {
    const existingVote = await voteRepository.findUserVoteByCategory({
      userId: idUsuario,
      ceremonyId: idCeremonia,
      categoryId
    });
    ...
    if (!existingVote) {
      throw new HttpError(404, "No has votado en esta categoria todavia.");
    }

    if (existingVote.nominacionId.toString() === nominacionId) {
      throw new HttpError(409, "Ya votaste por esta nominacion.");
    }

    const updated = await voteRepository.updateNominacionId({
      userId: idUsuario,
      ceremonyId: idCeremonia,
      categoryId,
      nominacionId
