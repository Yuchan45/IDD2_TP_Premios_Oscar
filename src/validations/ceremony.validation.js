...
function exactlyOneNominationTarget(value, { req, path }) {
  ...
  if (hasMovie === hasProfessional) {
    throw new Error("Cada nominacion debe incluir exactamente una pelicula o un profesional.");
  }
...
function validAwardWinner(value, { req, path }) {
  ...
  if (hasMovie === hasProfessional) {
    throw new Error("Cada premio debe incluir exactamente una pelicula o un profesional ganador.");
  }

  if (award.ganador.tipo === "pelicula" && !hasMovie) {
    throw new Error("Si ganador.tipo es pelicula, debe informarse ganador.pelicula.");
  }

  if (award.ganador.tipo === "profesional" && !hasProfessional) {
    throw new Error("Si ganador.tipo es profesional, debe informarse ganador.profesional.");
  }
...
const ceremonyPayloadValidation = [
  ...
  body("actuaciones.*.artistas.*.nombre").optional().isString().trim().notEmpty(),
  body("actuaciones.*.artistas.*.tipo").optional().isString().trim().isIn(validArtistTypes),
  body("nominaciones").optional().isArray(),
  body("nominaciones.*.categoria.id").optional().isMongoId(),
  body("nominaciones.*.categoria.nombre").optional().isString().trim().notEmpty(),
  body("nominaciones.*.pelicula.id").optional().isMongoId(),
  body("nominaciones.*.pelicula.titulo").optional().isString().trim().notEmpty(),
  body("nominaciones.*.profesional.id").optional().isMongoId(),
  body("nominaciones.*.profesional.nombreCompleto").optional().isString().trim().notEmpty(),
  ...
  body("premios.*.ganador.tipo").optional().isString().trim().isIn(validWinnerTypes),
  body("premios.*.ganador.pelicula.id").optional().isMongoId(),
  body("premios.*.ganador.pelicula.titulo").optional().isString().trim().notEmpty(),
  body("premios.*.ganador.profesional.id").optional().isMongoId(),
  body("premios.*.ganador.profesional.nombreCompleto").optional().isString().trim().notEmpty(),
